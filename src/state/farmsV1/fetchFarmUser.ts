...
export const fetchFarmUserAllowances = async (account: string, farmsToFetch: SerializedFarmConfig[], chainId) => {
...
export const fetchFarmUserTokenBalances = async (account: string, farmsToFetch: SerializedFarmConfig[], chainId) => {
  const calls = farmsToFetch.map((farm) => {
    const lpContractAddress = getAddress(farm.lpAddresses)
    return {
...
export const fetchFarmUserEarnings = async (account: string, farmsToFetch: SerializedFarmConfig[], chainId) => {
  const masterChefAddress = getMasterChefV1Address()
