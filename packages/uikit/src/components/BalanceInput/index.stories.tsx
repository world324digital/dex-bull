...
export const SwitchUnits: React.FC<React.PropsWithChildren> = () => {
  const CAKE_PRICE = 69;
  const [editingUnit, setEditingUnit] = useState<"BULL" | "USD">("BULL");
  const conversionUnit = editingUnit === "BULL" ? "USD" : "BULL";
  ...
  const switchEditingUnits = () => {
    const editingUnitAfterChange = editingUnit === "BULL" ? "USD" : "BULL";
  ...
  const handleCakeChange = (input: string) => {
    ...
    if (editingUnit === "BULL") {
      ...
      setValues({
        BULL: Number.isNaN(inputAsFloat) ? "" : `${inputAsFloat / CAKE_PRICE}`,
        USD: input,
      });
    }
  };
